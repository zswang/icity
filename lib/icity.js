(function (root, factory) {
    ...
    function parse(phone) {
        ...
        //  012  3456
        // [131][1234][5678]
        var providerCode = phone.slice(0, 3);
        var provider = providers[providerCode] || virtualProviders[phone.slice(0, 4)];
        if (!provider) {
            return null;
        }
        if (!cityBuffer[providerCode]) {
            var filename = path.join(__dirname, "../data/" + providerCode + ".dat");
            cityBuffer[providerCode] = fs.readFileSync(filename);
        }
        var offset = parseInt(phone.slice(3, 7), 10) * 2;
        var areaCode = cityBuffer[providerCode].readUInt16LE(offset);
        var cityInfo = citys[areaCode];
        return {
            province: cityInfo[0],
            city: cityInfo[1],
            areaCode: "0" + areaCode,
